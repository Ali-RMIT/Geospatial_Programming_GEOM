...
class DataPlotlyPanelWidget(QgsPanelWidget, WIDGET):  # pylint: disable=too-many-lines,too-many-instance-attributes,too-many-public-methods
    ...
    def __init__(self, mode=MODE_CANVAS, parent=None, override_iface=None, message_bar: QgsMessageBar = None):  # pylint: disable=too-many-statements
        ...
        self.properties_group_box.collapsedStateChanged.connect(
            self.refreshWidgets)
        ...
        self.add_to_layout_btn.clicked.connect(self.add_to_layout)
        self.save_plot_btn.setIcon(GuiUtils.get_icon('save_as_image.svg'))
        self.add_to_layout_btn.setIcon(GuiUtils.get_icon('add_to_layout.svg'))
    ...
    def add_to_layout(self):
        ...
        # Delete existing Layouts with the Same Name:
        layouts_list = manager.printLayouts()
        for ly in layouts_list:
        ...
        p_collection = layout.pageCollection()
        p_collection.pages()[0].setPageSize(
            'A3', QgsLayoutItemPage.Orientation.Landscape)
        layout.setName(layout_name)

        # # Add Map Layer to Layout..
        map_layer = QgsLayoutItemMap(layout)
        map_layer.setRect(QRectF(100, 100, 100, 100))  # map item size

        map_settings = QgsMapSettings()  # get Map Settings
        selected_layers = iface.layerTreeView().selectedLayers()  # Get Selected Layer
        map_settings.setLayers([selected_layers[0]])  # add layer to map

        # Set map Extent
        map_rectangle = QgsRectangle(map_settings.fullExtent())
        map_rectangle.scale(1.0)
        map_layer.setExtent(map_rectangle)

        # Control map item position on Layout
        map_layer.attemptMove(QgsLayoutPoint(
            5, 10, QgsUnitTypes.LayoutMillimeters))
        map_layer.attemptResize(QgsLayoutSize(
            260, 180, QgsUnitTypes.LayoutMillimeters))

        # Create Layout Image

        OUTPUT_PATH = os.getcwd()

        img = self.get_graph_image(OUTPUT_PATH)
        # add Image to Layout
        layout_image = QgsLayoutItemPicture(layout)
        layout_image.setPicturePath(img)

        # Move Image 5 from left and 100 from Top
        layout_image.attemptMove(QgsLayoutPoint(
            5, 100, QgsUnitTypes.LayoutMillimeters))
        # Image Item Size On The Layout (width , height)
        layout_image.attemptResize(QgsLayoutSize(
            270, 200, QgsUnitTypes.LayoutMillimeters))

        # finally add Map and Photo To layout
        layout.addLayoutItem(map_layer)
        layout.addLayoutItem(layout_image)

        # Add Layout to Project
        manager.addLayout(layout)
        self.iface.openLayoutDesigner(layout)

    def get_graph_image(self, path):
        '''
        Method to generate a png image from plot view
        and return path of saved image
        '''
        frame = self.plot_view.page().mainFrame()
        image = QImage(self.plot_view.page().viewportSize(),
                       QImage.Format_ARGB32)
        ...
        frame.render(painter)
        painter.end()
        image.save(path + os.sep + "plot_image.png")
        return path + os.sep + "plot_image.png"
